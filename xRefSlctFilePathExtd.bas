Attribute VB_Name = "xRefSlctFilePathExtd"
...
'◆ModuleProc名_ファイルを選択してそのパスを取得してシートに書き出す
'Rev.001
  
'//モジュールメモリ
  Private Const meMstrMdlName As String = "xRefSlctFilePathExtd"
...
'End Sub
'◆ModuleProc名_ファイルを選択してそのパスを取得してシートに書き出す
Private Sub callxRefSlctFilePathExtd()
'  Dim myXlonOutputOptn As Long, _
'        myXlonFDType As Long, myXstrDfltFldrPath As String, myXstrDfltFilePath As String, _
...
'    'myZstrFileName(i) : ファイル名
'    'myZstrFilePath(i) : ファイルパス
    Call xRefSlctFilePathExtd.callProc( _
            myXbisCmpltFlag, _
...
'
Private Sub resetConstantInxRefSlctFilePathExtd()
'//xRefSlctFilePathExtdモジュールのモジュールメモリのリセット処理
    Call xRefSlctFilePathExtd.resetConstant
End Sub
