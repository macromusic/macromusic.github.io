Attribute VB_Name = "xRefSlctFilePathRpt"
'Includes CSlctFilePathRpt
'Includes PincPickUpExtensionMatchFilePath2DArray
...
'◆ModuleProc名_指定文字列を含むファイル名のファイルを繰返し選択してそのパスを取得する
'Rev.001
  
'//モジュールメモリ
  Private Const meMstrMdlName As String = "xRefSlctFilePathRpt"
  Private Const meMlonExeNum As Long = 0
  
'//出力データ
  Private myXlonFileCnt As Long, myZobjFile() As Object, _
            myZstrFileName() As String, myZstrFilePath() As String
    'myZobjFile(i, 1) : ファイルオブジェクト
...
Public Sub callProc( _
            ...
            ByRef myZvarOdrTrgtWrdPosIN() As Variant)
...
'End Sub
'◆ModuleProc名_指定文字列を含むファイル名のファイルを繰返し選択してそのパスを取得する
Private Sub callxRefSlctFilePathRpt()
'  Dim myXstrDfltFldrPath As String, myXstrDfltFilePath As String, _
...
'        myXlonOrdrCnt As Long, myXlonTrgtWrdCnt As Long, _
'        myZvarOdrTrgtWrdPos() As Variant
'    'myXstrDfltFldrPath = ""  : デフォルトパス指定無し
'    'myXstrDfltFldrPath = "C" : Cドライブをデフォルトパスに指定
'    'myXstrDfltFldrPath = "1" : このブックの親フォルダをデフォルトパスに指定
'    'myXstrDfltFldrPath = "2" : アクティブブックの親フォルダをデフォルトパスに指定
...
'    'myZstrFileName(i, 1) : ファイル名
'    'myZstrFilePath(i, 1) : ファイルパス
    Call xRefSlctFilePathRpt.callProc( _
            ...
            myXlonOrdrCnt, myXlonTrgtWrdCnt, myZvarOdrTrgtWrdPos)
...
'
Private Sub resetConstantInxRefSlctFilePathRpt()
'//xRefSlctFilePathRptモジュールのモジュールメモリのリセット処理
    Call xRefSlctFilePathRpt.resetConstant
End Sub
