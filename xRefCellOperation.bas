Attribute VB_Name = "xRefCellOperation"
...
'◆ModuleProc名_エクセルシート内の指定セル範囲に対して処理を実行する
'Rev.001
  
'//モジュールメモリ
  Private Const meMstrMdlName As String = "xRefCellOperation"
...
'//モジュール内変数_データ
  Private myXobjRng As Object
  Private myXlonCellCnt As Long, myXlonErrCellCnt As Long, myZobjErrCell() As Object
...
'//エクセルシート内の指定セル範囲に対して処理を実行
    Call PabsCellOperation( _
            myXbisExitFlag, myXlonCellCnt, myXlonErrCellCnt, myZobjErrCell, _
            myXobjRng)
...
End Sub
 ...
 '抽象Ｐ_エクセルシート内の指定セル範囲に対して処理を実行する
Private Sub PabsCellOperation( _
            myXbisExitFlag As Boolean, myXlonCellCnt As Long, _
            myXlonErrCellCnt As Long, myZobjErrCell() As Object, _
            ByVal myXobjRng As Object)
    myXlonCellCnt = Empty: myXlonErrCellCnt = Empty: Erase myZobjErrCell
    On Error GoTo ExitPath
  Dim k As Long: k = myXobjRng.Cells.Count
    On Error GoTo 0
  Dim myZlonTmp(1) As Long, Lo As Long: Lo = LBound(myZlonTmp)
  Dim myXobjCell As Object, n As Long, e As Long: n = 0: e = Lo - 1
    For Each myXobjCell In myXobjRng
        Call PsubCellOperation(myXbisExitFlag, myXobjCell)
        If myXbisExitFlag = True Then
            e = e + 1: ReDim Preserve myZobjErrCell(e) As Object
            Set myZobjErrCell(e) = myXobjCell
        Else
            n = n + 1
        End If
    Next myXobjCell
    myXlonCellCnt = n: myXlonErrCellCnt = e - Lo + 1
    If myXlonErrCellCnt >= 1 Then
        myXbisExitFlag = True
    Else
        myXbisExitFlag = False
    End If
    Set myXobjCell = Nothing
    Exit Sub
ExitPath:
    myXbisExitFlag = True
End Sub
Private Sub PsubCellOperation(myXbisExitFlag As Boolean, _
...
'◆ModuleProc名_エクセルシート内の指定セル範囲に対して処理を実行する
Private Sub callxRefCellOperation()
  Dim myXbisCompFlag As Boolean
    Call xRefCellOperation.callProc(myXbisCompFlag)
...
'
Private Sub resetConstantInxRefCellOperation()
'//xRefCellOperationモジュールのモジュールメモリのリセット処理
    Call xRefCellOperation.resetConstant
End Sub
