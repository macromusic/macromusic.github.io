Attribute VB_Name = "xRefObjectOperation"
...
'◆ModuleProc名_エクセルブック内の任意の同一全オブジェクトに対して処理を実行する
'Rev.001
  
'//モジュールメモリ
  Private Const meMstrMdlName As String = "xRefObjectOperation"
...
'//モジュール内変数_データ
  Private myXobjOrg As Object
  Private myXlonOrgCnt As Long, myXlonErrOrgCnt As Long, myZobjErrOrg() As Object

'iniP_モジュール内変数を初期化する
Private Sub initializeModuleVariables()
    myXbisExitFlag = False
    
    Set myXobjOrg = Nothing
    myXlonOrgCnt = Empty
    myXlonErrOrgCnt = Empty
    Erase myZobjErrOrg
End Sub
...
'//エクセルブック内の任意の同一全オブジェクトに対して処理を実行
    Call PabsObjectOperation( _
            myXbisExitFlag, myXlonOrgCnt, myXlonErrOrgCnt, myZobjErrOrg, _
            myXobjOrg)
...
End Sub
 ...
 '抽象Ｐ_エクセルブック内の任意の同一全オブジェクトに対して処理を実行する
Private Sub PabsObjectOperation( _
            myXbisExitFlag As Boolean, myXlonOrgCnt As Long, _
            myXlonErrOrgCnt As Long, myZobjErrOrg() As Object, _
            ByVal myXobjOrg As Object)
    myXlonOrgCnt = Empty: myXlonErrOrgCnt = Empty: Erase myZobjErrOrg
    On Error GoTo ExitPath
    If myXobjOrg Is Nothing Then GoTo ExitPath
    On Error GoTo 0
  Dim myZlonTmp(1) As Long, Lo As Long: Lo = LBound(myZlonTmp)
  Dim myXobjTmp As Object, n As Long, e As Long: n = 0: e = Lo - 1
    For Each myXobjTmp In myXobjOrg.Worksheets
        Call PsubObjectOperation(myXbisExitFlag, myXobjTmp)
        If myXbisExitFlag = True Then
            e = e + 1: ReDim Preserve myZobjErrOrg(e) As Object
            Set myZobjErrOrg(e) = myXobjTmp
        Else
            n = n + 1
        End If
    Next myXobjTmp
    myXlonOrgCnt = n: myXlonErrOrgCnt = e - Lo + 1
    If myXlonErrOrgCnt >= 1 Then
        myXbisExitFlag = True
    Else
        myXbisExitFlag = False
    End If
...
ExitPath:
    myXbisExitFlag = True
End Sub
Private Sub PsubObjectOperation(myXbisExitFlag As Boolean, _
            ByVal myXobjTmp As Object)
    myXbisExitFlag = False
'//ブック内の任意の同一全オブジェクトに対する処理
'    XarbProgCode
    Call MsubProc
    Exit Sub
ExitPath:
...
'◆ModuleProc名_エクセルブック内の任意の同一全オブジェクトに対して処理を実行する
Private Sub callxRefObjectOperation()
  Dim myXbisCompFlag As Boolean
    Call xRefObjectOperation.callProc(myXbisCompFlag)
...
'
Private Sub resetConstantInxRefObjectOperation()
'//xRefObjectOperationモジュールのモジュールメモリのリセット処理
    Call xRefObjectOperation.resetConstant
End Sub
