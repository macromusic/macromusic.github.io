Attribute VB_Name = "m1Msub3SubFldrLstExtd"
...
'//モジュールメモリ
  Private Const meMstrMdlName As String = "m1Msub3SubFldrLstExtd"
...
'//出力データ
  ...
  Private myXobjFldrPstdCell As Object
...
'//処理実行
    Call callm1Msub3SubFldrLstExtd
...
Private Sub instCVrblToSht()
  Dim myXinsVrblToSht As CVrblToSht: Set myXinsVrblToSht = New CVrblToSht
    ...
    '//クラス内プロシージャの実行とクラス内変数からの出力
        ...
        Set myXobjFldrPstdCell = .getPstdRng
    End With
...
'End Sub
'◆ModuleProc名_指定ディレクトリ内のサブフォルダ一覧を取得してシートに書き出す
Private Sub callm1Msub3SubFldrLstExtd()
...
'        myZstrFldrName() As String, myZstrFldrPath() As String, myXobjFldrPstdCell As Object
'    'myZobjFldr(k) : フォルダオブジェクト
'    'myZstrFldrName(k) : フォルダ名
'    'myZstrFldrPath(k) : フォルダパス
...
'
Private Sub resetConstantInm1Msub3SubFldrLstExtd()
'//m1Msub3SubFldrLstExtdモジュールのモジュールメモリのリセット処理
    Call m1Msub3SubFldrLstExtd.resetConstant
End Sub
